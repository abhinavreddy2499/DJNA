...
def index(request):
    if bool(request):
        ...
        start_coordinates = [gmaps.geocode(start_location)[0]['geometry']['location']['lat'],
                             gmaps.geocode(start_location)[0]['geometry']['location']['lng']]
        end_coordinates = [gmaps.geocode(end_location)[0]['geometry']['location']['lat'],
                           gmaps.geocode(end_location)[0]['geometry']['location']['lng']]
