...
def index(request):
    if bool(request):
        ...
        logger.info(
            f'Request received for parameters: Source = {source_loc}, Destination = {dest_loc}, Max percent : {percentage}, min_max_elevation_gain : {minmax_elev_gain}')

        start_coordinates = [gmaps.geocode(source_loc)[0]['geometry']['location']['lat'],
                             gmaps.geocode(source_loc)[0]['geometry']['location']['lng']]
        end_coordinates = [gmaps.geocode(dest_loc)[0]['geometry']['location']['lat'],
                           gmaps.geocode(dest_loc)[0]['geometry']['location']['lng']]

        view = MapView()
        controller = RouteController()

        controller.get_routes(start_coordinates, end_coordinates, percentage, minmax_elev_gain, view)
    ...
    return jsonify(origin=start_coordinates, des=end_coordinates, path=path, dis=total_distance,
                   elev=calculated_elevation)
